...
final class JarURLConnection extends java.net.JarURLConnection {
   ...
   static JarURLConnection get(URL url, JarFile jarFile) throws IOException {
      ...
      if (index == -1) {
         return Boolean.TRUE.equals(useFastExceptions.get()) ? NOT_FOUND_CONNECTION : new JarURLConnection(url, (AbstractJarFile)null, EMPTY_JAR_ENTRY_NAME);
      } else {
         int separator;
   ...
   static class JarEntryName {
      ...
      private void write(String source, ByteArrayOutputStream outputStream) {
         ...
         for(int i = 0; i < length; ++i) {
            ...
            if (c > 127) {
               ...
               if (c == '%') {
                  if (i + 2 >= length) {
                     String var10002 = source.substring(i);
      ...
      static JarEntryName get(StringSequence spec, int beginIndex) {
         return spec.length() <= beginIndex ? JarURLConnection.EMPTY_JAR_ENTRY_NAME : new JarEntryName(spec.subSequence(beginIndex));
      }
   }
}
